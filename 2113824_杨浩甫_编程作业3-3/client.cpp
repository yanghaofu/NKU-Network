...
// 定义超时时间
std::chrono::milliseconds timeout(500); // 800毫秒示例超时时间
// 记录每个数据包的发送时间
...
void receiveACK() {
    ...
    while (isRunning) {
        message ackMsg = recvmessage(client, serveraddr);
        if (ackMsg.isACK() && ackMsg.isEXT()) {
            std::lock_guard<std::mutex> lock(mtx); // 在代码块中自动上锁，出作用域时自动释放
            int ackNum = ackMsg.ack;
            acked[(ackNum - 1) % WINDOW_SIZE] = false;
            timeused[(ackNum - 1) % WINDOW_SIZE] = false;
            while (base < ackNum && !acked[base % WINDOW_SIZE]) {
                (base)++;
            }
            //cout << endl;
            //// 输出当前窗口占用情况
            ...
            //    if (acked[j % WINDOW_SIZE] && j < nextSeqNum) {
            //        SetColor(14, 0);
            //        cout << "[ " << i << " ]" << " ";
            //    }
            //    else {
...
void timeoutResend() {
    while (isRunning) {
        ...
        for (int i = base; i < nextSeqNum; ++i) {
            ...
            // 判断单个数据包是否超时
            if (((clock() - packetTimes[index]) > timeout.count()) && timeused[i]) {
                std::lock_guard<std::mutex> lock(mtx); // 在代码块中自动上锁，出作用域时自动释放
        ...
        // 延时一段时间再次检查
        std::this_thread::sleep_for(std::chrono::milliseconds(500)); // 假设延时100毫秒
    }
}
...
void SendFile(const char* filepath) {
    ...
    // 创建线程并启动
    std::thread t(timeoutResend);
    t.detach();

    while (filelen || base != nextSeqNum-1) {
        ...
        if (nextSeqNum < base + WINDOW_SIZE && filelen > 0) {
            ...
            cout << "Client:此时已发送数据右端在窗口的位置: " << nextSeqNum - base << endl;

            msg.output();

            timeused[nextSeqNum] = true;
